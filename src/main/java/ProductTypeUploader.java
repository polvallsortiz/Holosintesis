...
public class ProductTypeUploader extends Component {


    private Stage primaryStage;
    private TextField productTypeTitle;
    private Button selectImage;
    private TextField productTypeDescription;
    private File productTypeImage;
    ...
    public ProductTypeUploader(Stage primaryStage) throws IOException {
        ...
        productTypeTitle = (TextField) primaryStage.getScene().lookup("#productTypeTitle");
        selectImage = (Button) primaryStage.getScene().lookup("#selectImage");
        productTypeDescription = (TextField) primaryStage.getScene().lookup("#productTypeDescription");
        ...
        productTypeLabel = (Label) primaryStage.getScene().lookup("#productTypeLabel");
    ...
    private void getImage() {
        ...
        if (result == JFileChooser.APPROVE_OPTION) {
            ...
            productTypeImage = fc.getSelectedFile();
        }
    }

    private void pushProductType() throws IOException, JSchException {
       // if(!productTypeTitle.getText().isEmpty() && !productTypeDescription.getText().isEmpty() && !designPatologies.getText().isEmpty() && productTypeImage == null) {
            pushImage();
            String title = productTypeTitle.getText();
            String description = productTypeDescription.getText();
            String imageURL = "http://holosintesis.ddns.net/DataBase/ProductType/" + productTypeImage.getName();

            Map<String,Object> params = new LinkedHashMap<>();
            params.put("title_producttype",title);
            params.put("image_producttype",imageURL);
            params.put("description_producttype",description);

            StringBuilder tokenUri=new StringBuilder("title_producttype=");
            tokenUri.append(URLEncoder.encode(title,"UTF-8"));
            tokenUri.append("&image_producttype=");
            tokenUri.append(URLEncoder.encode(imageURL,"UTF-8"));
            tokenUri.append("&description_producttype=");
            ...
            productTypeLabel.setText(title);
    ...
    private  void pushImage() throws JSchException, IOException {
        File selectedFile = productTypeImage;
        String user = "holosintesisserver";
        String host = "holosintesis.ddns.net";
        String rfile = "/var/www/html/DataBase/ProductType/" + selectedFile.getName();
